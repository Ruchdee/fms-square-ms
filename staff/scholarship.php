<?php
    ...
    //check session - staff id, name, e-mail, image profile
    ...
    $active_stype = true;
    $result_stype = $stype->readall($active_stype);
    $result_stype_add = $stype->readall($active_stype);

    //read all records from scholarship table
    $active = false;
    $result = $scholarhsip->readall($active);
...
<head>
    ...
    <title>ข้อมูลทุนการศึกษา-เจ้าหน้าที่ | FMS Student Portal</title>
    <!-- Custom CSS -->
...
<body>
            ...
            <div class="page-breadcrumb border-bottom">
                <div class="row">
                    <div class="col-lg-6 col-md-6 col-xs-12 align-self-center">
                        <h4 class="font-medium text-uppercase mb-0">ข้อมูลทุนการศึกษา</h4>
                    </div>
                    <div class="col-lg-6 col-md-6 col-xs-12 align-self-center">
            ...
            <div class="email-app todo-box-container">
	            ...
	            <div class="left-part list-of-tasks">
	                ...
	                <div class="scrollable" style="height:100%;">
	                    <div class="p-3">
	                        <a class="waves-effect waves-light btn btn-info d-block" href="javascript: void(0)" id="add-scholarship"><i class="mdi mdi-plus-box font-16 mr-1"></i>เพิ่มข้อมูล</a>
	                    </div>
	                    <div class="divider"></div>
	                    <ul class="list-group">
	                        <li>
	                            <small class="p-3 grey-text text-lighten-1 db">ประเภททุนการศึกษา</small>
	                        </li>
                            ...
                            <?php 
                                while ($row_stype = mysqli_fetch_array($result_stype)) { ?>
                                    <li class="list-group-item">
                                        <a href="javascript:void(0)" class="todo-link list-group-item-action" id="<?php echo $row_stype['scholarship_type_id']; ?>"> <i class="mdi mdi-link-variant"></i> <?php echo $row_stype['scholarship_type_name']; ?> <span class="todo-badge badge badge-warning float-right"></span></a>
                                    </li>
                            <?php } ?>
	                    </ul>
	                </div>
	                    ...
	                    <div class="todo-listing">
                            ...
                            <?php if (mysqli_num_rows($result) > 0) { ?>
                                <?php while ($row = mysqli_fetch_array($result)) { ?>
                                    ...
                                    ?>
                                    <div class="todo-item all-news-list p-3 border-bottom position-relative <?php echo $row['scholarship_type_id']; ?>">
                                        <div class="inner-item d-flex align-items-start">
                                            <div class="w-100">
                                                <div class="custom-control d-flex align-items-start">
                                                    <div>
                                                        <div class="content-todo">
                                                            <h5 class="font-medium font-16 todo-header mb-0" data-todo-header="<?php echo $row['scholarship_name']; ?>"><?php echo $row['scholarship_name']; ?> <i class="mdi <?php echo $istatus; ?>"></i></h5>
                                                            <div class="todo-subtext text-muted" 
                                                                data-todosubtext-html="<?php echo substr(htmlspecialchars(trim(strip_tags($row['scholarship_desc']))), 0, 500); ?>" data-todosubtextText="">
                                                                <?php 
                                                                    //echo trim($row['news_desc']);
                                                                    if (strlen(trim($row['scholarship_desc'])) > 500) {
                                                                        echo substr(htmlspecialchars(trim(strip_tags($row['scholarship_desc']))), 0, 500) . "...";
                                                                    } else {
                                                                        echo trim($row['scholarship_desc']);
                                                                    }
                                                                ?>
                                                            </div>
                                                            <span class="todo-time font-14 text-muted"><i class="icon-calender mr-1"></i><?php echo $row['scholarship_date']; ?></span>
                                                        </div>
                                                    </div>
                                                    <div class="ml-auto">
                                                        <div class="dropdown-action">
                                                            <div class="dropdown todo-action-dropdown">
                                                                ...
                                                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="more-action-1">
                                                                    <a class="edit dropdown-item" href="#" data-toggle="modal" data-target="#updateModal" data-id="<?php echo $row['scholarship_id']; ?>" id="btn-update"><i class="fas fa-edit text-info mr-1"></i> แก้ไข</a>
                                                                    <a class="remove dropdown-item" href="#" data-id="<?php echo $row['scholarship_id']; ?>" id="btn-delete"><i class="far fa-trash-alt text-danger mr-1"></i> ลบ</a>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                ...
                                <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header bg-info text-white">
                                            <h5 class="modal-title">แก้ไข-ข้อมูลทุนการศึกษา</h5>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <span aria-hidden="true">&times;</span>
                                            </button>
                                        </div>
                                        <form id="form-update" method="post" action="scholarship_op.php?action=u">
                ...
                <!-- Modal -->
                <div class="modal fade" id="addScholarshipModal" tabindex="-1" role="dialog" aria-labelledby="addTaskModalTitle" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                        <div class="modal-content border-0">
                        <form id="form-add" class="form-horizontal" method="post" action="scholarship_op.php?action=i">
                            <div class="modal-header bg-info text-white">
                                <h5 class="modal-title">เพิ่มข้อมูลทุนการศึกษา</h5>
                                ...
                                <div class="compose-box">
                                    <div class="compose-content" id="addTaskModalTitle">
                                                ...
                                                <select class="form-control custom-select" id="scholarship-type" name="scholarship-type" required>
                                                    <?php
                                                        while ($row_stype_add = mysqli_fetch_array($result_stype_add)) {
                                                            echo "<option value='" . $row_stype_add['scholarship_type_id'] ."'>" . $row_stype_add['scholarship_type_name'] . "</option>";
                                                        } 
                                                    ?>
                                                </select>
                                            </div>
                                            <div class="col-md-3">
                                                <input type="date" class="form-control" id="scholarship-date" name="scholarship-date" required><span class="validation-text"></span>
                                            </div>
                                            <div class="col-md-3">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="d-flex mail-to mb-4">
                                                    <div class="w-100">
                                                        <input id="scholarship-name" name="scholarship-name" type="text" placeholder="ชื่อทุนการศึกษา" class="form-control" required maxlength="200">
                                        ...
                                        <div class="d-flex mail-subject mb-4">
                                            <div class="w-100">
                                                <div id="scholarship-desc" class=""></div>
                                                <textarea id="scholarship-desc-txt" name="scholarship-desc-txt" style="display:none"></textarea>
                                                <span class="validation-text"></span>
                                            </div>
                                        </div>
                                    </div>
    ...
    <script src="../dist/js/scholarship.js"></script>
    ...
    <script>
        $(document).ready(function(){	
            $(document).on('click', '#btn-update', function(e){
                e.preventDefault();
                var scholarship_id = $(this).data('id');   // it will get id of clicked row
                //console.log(stype_id);
                $('#dynamic2-content').html(''); // leave it blank before ajax call
                $('#modal-loader').show();      // load ajax loader
                $.ajax({
                    url: 'scholarship_update.php',
                    type: 'POST',
            ...
            $(document).on('click', '#btn-delete', function(e){
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $(location).attr("href", "scholarship_op.php?action=d&scholarship_id=" + scholarship_id);
                        Swal.fire(
                            'Deleted!',
...
<?php
    if (isset($_GET['msg'])) {
        switch ($_GET['msg']) {
            case 'success':
                echo "<script>toastr.success('บันทึกข้อมูลเรียบร้อยแล้ว', 'ข้อมูลทุนการศึกษา');</script>";
                break;
            case 'insert-error':
                echo "<script>toastr.error('เพิ่มข้อมูลผิดพลาด', 'ข้อมูลทุนการศึกษา');</script>";
                break;
            case 'update-error':
                echo "<script>toastr.error('แก้ไขข้อมูลผิดพลาด', 'ข้อมูลทุนการศึกษา');</script>";
                break;
            case 'delete-error': 
                echo "<script>toastr.error('ลบข้อมูลผิดพลาด', 'ข้อมูลทุนการศึกษา');</script>";
                break;
        }
    }
?>