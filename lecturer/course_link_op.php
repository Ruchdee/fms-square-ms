<?php 
    ...
    $clink_update->created_by = $_SESSION['lecturer_ppid'];
    $clink_update->created_date = date("Y/m/d H:i:s");

    $resut = $clink_update->readone();
    if (mysqli_fetch_array($resut)) {
        //update
        if ($clink_update->update()) {
            header("Location: course_link.php?msg=success");
        } else {
            header("Location: course_link.php?msg=update-error");
        }
    } else {
        //insert
        $clink_update->lecturer_pers_id = $_SESSION['lecturer_id_card'];
        if ($clink_update->create()) {
            header("Location: course_link.php?msg=success");
        } else {
            header("Location: course_link.php?msg=insert-error");
        }
    }
?>