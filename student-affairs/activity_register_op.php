<?php 
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'i') {
        $activity_register->activity_id = $_POST['activity-id'];
        $activity_register->student_id = $_SESSION['std_id'];
        $activity_register->student_email = $_POST['student-email'];
        $activity_register->student_phone = $_POST['student-phone'];
        $activity_register->registered_date = date("Y/m/d H:i:s");
        //insert
        if ($activity_register->create()) {
            header("Location: activity_list.php?msg=success");
        } else {
            header("Location: activity_list.php?msg=insert-error");
       }
    }
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'u') {
        ...
        if ($activity_register->update()) {
            ...
            header("Location: activity_list.php?msg=update-error");
        }
    }
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'd') {
        if (isset($_GET['activity_id'])) {
            ...
            if ($activity_register->delete()) {
                ...
                header("Location: activity_list.php?msg=delete-error");
            }
        }
    }
