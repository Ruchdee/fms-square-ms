<?php
    ...
    if (isset($_POST['btn-search'])) {
        ...
        switch ($_POST['search-type']) {
            case 'subject-code':
                $course_open->subject_code = $_POST['search-txt'];
                $result_copen = $course_open->readonebysubject_code();
                break;
            case 'subject-name-thai':
                $course_open->subject_name_thai = $_POST['search-txt'];
                $result_copen = $course_open->readonebysubject_name_thai();
                break;
            case 'subject-name-eng':
                $course_open->subject_name_eng = $_POST['search-txt'];
                $result_copen = $course_open->readonebysubject_name_eng();
                break;
            case 'lecturer-name':
                # code...
                break;
...
<head>
	...
	<title>ข้อมูลลิงค์รายวิชา | FMS Student Portal</title>
	<!-- Custom CSS -->
...
<body>
	...
	<div id="main-wrapper">
		...
		<div class="page-wrapper">
                    ...
                    <ul class="nav nav-pills p-3 bg-white mb-3 rounded-pill align-items-center">
                        ...
                        <div class="col-md-3">
                            <select class="form-control custom-select mr-sm-2" id="search-type" name="search-type">
                                <option value="subject-code" <?php if(isset($_POST['search-type']) && $_POST['search-type'] == 'subject-code') echo "selected"; ?>>รหัสรายวิชา</option>
                                <option value="subject-name-thai" <?php if(isset($_POST['search-type']) && $_POST['search-type'] == 'subject-name-thai') echo "selected"; ?>>ชื่อรายวิชา (ไทย)</option>
                                <option value="subject-name-eng" <?php if(isset($_POST['search-type']) && $_POST['search-type'] == 'subject-name-eng') echo "selected"; ?>>ชื่อรายวิชา (Eng)</option>
                                <!-- <option value="lecturer-name" <?php if(isset($_POST['search-type']) && $_POST['search-type'] == 'lecturer-name') echo "selected"; ?>>ชื่ออาจารย์ผู้สอน</option> -->
                            </select>
                        </div>
                        <div class="input-group mr-sm-2 col-md-7">
                            ...
                            <input type="text" class="form-control mr-2" id="search-txt" name="search-txt" placeholder="ข้อความที่ต้องการค้นหา" required value="<?php if (isset($_POST['search-txt'])) echo $_POST['search-txt']; ?>">
                            <button type="submit" class="btn waves-effect waves-light btn-rounded btn-primary" id="btn-search" name="btn-search"> ค้นหา..</button>
                        </div>
                        <div class="col-md-1"></div>
                    </ul>
                </form>
                                    ...
                                    <?php
                                        if ($activity->chkExpired($row['activity_date'])) {
                                            ...
                                            if (!mysqli_fetch_array($result_act_register)) {
                                                echo "<a href='#' class='btn btn-info btn-rounded' data-id='" . $row['activity_id'] . "' id='btn-register'>ลงทะเบียน</a>";
                                            } else {
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped search-table v-middle">
                            <thead class="header-item">
                                <th class="text-center text-dark font-weight-bold">รหัสรายวิชา</th>
                                <th class="text-dark font-weight-bold">ชื่อรายวิชา</th>
                                <th class="text-center text-dark font-weight-bold">ตอน</th>
                                <th class="text-center text-dark font-weight-bold">หน่วยกิต</th>
                                <th class="text-dark font-weight-bold">อาจารย์ผู้สอน</th>
                                <th class="text-center text-dark font-weight-bold">ลิงค์รายวิชา</th>
                            </thead>
                            <tbody>
                                <?php
                                    if (isset($result_copen)) {
                                        $cnt_result = 0;
                                        while ($row_copen = mysqli_fetch_array($result_copen)) {
                                            $course_link->section_offer_id = $row_copen['section_offer_id'];
                                            $result_clink = $course_link->readone();
                                            if ($row_clink = mysqli_fetch_array($result_clink)) {
                                                //get lecturer id from section_lecturer table
                                                $section_lect->section_offer_id = $row_clink['section_offer_id'];
                                                $result_slect = $section_lect->readonebysection_offer_id();
                                                $row_slect = mysqli_fetch_array($result_slect);
                                                //get lecturer name from lecturer table
                                                $lecturer->lecturer_pers_id = $row_slect['lecturer_id'];
                                ...
                                <tr class="search-items">
                                    <td class="text-center">
                                        <span class="user-work text-center"><?php echo $row_copen['subject_code']; ?></span>
                                    </td>
                                    <td>
                                        <span class="usr-email-addr">
                                            <?php echo $row_copen['subject_name_thai'] . " (" . $row_copen['subject_name_eng'] . ")"; ?>
                                        </span>
                                    </td>
                                    <td class="text-center">
                                        <span class="usr-location">
                                            <?php echo $row_copen['section']; ?> 
                                        </span>
                                    </td>
                                    <td class="text-center">
                                        <span class="usr-location">
                                            <?php echo $row_copen['course_credit']; ?> 
                                        </span>
                                    </td>
                                    <td>
                                        <span class="usr-location"> 
                                            <a href="#"><?php echo $title_lect[$row_lecturer['title_th']] . $row_lecturer['lecturer_name_th']; ?></a>
                                        </span>
                                    </td>
                                    <td class="text-center">
                                        <div class="action-btn">
                                            <a href="#" class="text-info" data-toggle="modal" data-target="#showLinkModal" data-id="<?php echo $row_clink['section_offer_id']; ?>" id="btn-link"><i class="mdi mdi-launch font-18"></i></a>
                                        </div>
                                    </td>
                                </tr>
                                <!-- /.row -->
                                ...
                                <?php }  // if ?>
                                <?php }  // while ?>
                                <?php if (isset($cnt_result) && $cnt_result == 0) { ?>
                                    <tr class="search-items">
                                        <td class="text-center" colspan="6"><span class="text-danger">ไม่มีข้อมูล</span></td>
                                    </tr>
                                <?php } ?>
                            </tbody>
                        </table>
            ...
            <!-- Show Course Links Modal -->
            <div class="modal fade" id="showLinkModal" tabindex="-1" role="dialog" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                        <div class="modal-content">
                            <div class="modal-header bg-info text-white">
                                <h5 class="modal-title">ลิงค์รายวิชา</h5>
    ...
    <script>
        $(document).ready(function() {
            $(document).on('click', '#btn-link', function(e){
                e.preventDefault();
                var section_offer_id = $(this).data('id');   // it will get id of clicked row
                $('#dynamic2-content').html(''); // leave it blank before ajax call
                $('#modal-loader').show();      // load ajax loader
                $.ajax({
                    url: 'course_link_detail.php',
                    type: 'POST',
